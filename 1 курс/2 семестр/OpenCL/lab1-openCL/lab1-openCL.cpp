...
const char* Source =
"__kernel void vectorAdd(__global float* w, __global const float* x, __global const float* y, float a, float b, __global const float* z) {\n"
"    int i = get_global_id(0);\n"
"    w[i] = a * x[i] + b * y[i] * z[i];\n"
"}\n";

/*const char* Source =
"__kernel void matrixVectorMult(__global float* w, __global const float* Y, __global const float* x, float a, int size) {\n"
"    int i = get_global_id(0);\n"
"    float sum = 0.0f;\n"
...
int main()
{
	...
	cl_uint uNumGPU;
	clGetDeviceIDs(pPlatforms[platform_id], CL_DEVICE_TYPE_GPU, 1, &deviceID, &uNumGPU);
	...
	cl_program program = clCreateProgramWithSource(context, 1, &Source, (const size_t*)&source_size, &errcode_ret);
	if (errcode_ret != CL_SUCCESS)
	{
	...
	//
	std::vector<float> x(vectorSize, 1.0f);
	std::vector<float> y(vectorSize, 2.0f);
	std::vector<float> z(vectorSize, 3.0f);
	std::vector<float> Y(vectorSize * vectorSize, 1.0f);
	float a = 2.0f;
	float b = 3.0f;
	...
	//
	errcode_ret = CL_SUCCESS;
	cl_mem bufferW = clCreateBuffer(
		context, CL_MEM_READ_WRITE,
		sizeof(float) * vectorSize, NULL, &errcode_ret);
	...
	cl_mem bufferY = clCreateBuffer(
		...
		sizeof(float) * vectorSize,
		y.data(), &errcode_ret);
	cl_mem bufferZ = clCreateBuffer(
		context,
		CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
		sizeof(float) * vectorSize,
		z.data(), &errcode_ret);
	cl_mem bufferYY = clCreateBuffer(
		context,
		CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
		sizeof(float) * vectorSize * vectorSize,
		Y.data(), &errcode_ret);
	if (errcode_ret != CL_SUCCESS)
	{
	...
	//
	// 12. Установка буфера в качестве аргумента ядра
	//
	errcode_ret = clSetKernelArg(kernel, 0, sizeof(cl_mem), &bufferW);
	errcode_ret = clSetKernelArg(kernel, 1, sizeof(cl_mem), &bufferX);
	errcode_ret = clSetKernelArg(kernel, 2, sizeof(cl_mem), &bufferY);
	errcode_ret = clSetKernelArg(kernel, 3, sizeof(float), &a);
	errcode_ret = clSetKernelArg(kernel, 4, sizeof(float), &b);
	errcode_ret = clSetKernelArg(kernel, 5, sizeof(cl_mem), &bufferZ);
	
	/*
	2 задание
	errcode_ret = clSetKernelArg(kernel, 0, sizeof(cl_mem), &bufferW);
	errcode_ret = clSetKernelArg(kernel, 1, sizeof(cl_mem), &bufferYY);
	errcode_ret = clSetKernelArg(kernel, 2, sizeof(cl_mem), &bufferX);
	errcode_ret = clSetKernelArg(kernel, 3, sizeof(float), &a);
	errcode_ret = clSetKernelArg(kernel, 4, sizeof(int), &vectorSize);
	*/
	...
	//
	errcode_ret = CL_SUCCESS;
	size_t uGlobalWorkSize = vectorSize;
	errcode_ret = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &uGlobalWorkSize, NULL, 0, NULL, NULL);
	
	/*float w[vectorSize];
	clEnqueueReadBuffer(queue, bufferW, CL_TRUE, 0, sizeof(float)* vectorSize, w, 0, NULL, NULL);
	*/
	clFinish(queue);
	if (errcode_ret != CL_SUCCESS)
	{
	...
	//
	// 14. Отображение буфера в память управляющего узла
	//
	errcode_ret = CL_SUCCESS;
	cl_int* puData = (cl_int*)clEnqueueMapBuffer(
		queue, buffer, CL_TRUE, CL_MAP_READ, 0,
		vectorSize * sizeof(cl_int), 0, NULL, NULL, &errcode_ret);
	if (errcode_ret != CL_SUCCESS)
	{
		printf("Error to create context");
		return 0;
	...
	//
	// 16. Завершение отображения буфера
	//
	errcode_ret = CL_SUCCESS;
	clEnqueueUnmapMemObject(
		queue, buffer, puData, 0, NULL, NULL);

	/*std::cout << "Result vector w:" << std::endl;
	for (int i = 0; i < vectorSize; ++i) {
		std::cout << w[i] << " ";
	}
	std::cout << std::endl;*/
	...
	//
	// 17. Удаление объектов и освобождение памяти 
	// управляющего узла
	//
